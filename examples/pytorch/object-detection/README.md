...
This directory contains 2 scripts that showcase how to fine-tune any model supported by the [`AutoModelForObjectDetection` API](https://huggingface.co/docs/flux/main/en/model_doc/auto#flux.AutoModelForObjectDetection) (such as [DETR](https://huggingface.co/docs/flux/main/en/model_doc/detr), [DETA](https://huggingface.co/docs/flux/main/en/model_doc/deta), [Deformable DETR](https://huggingface.co/docs/flux/main/en/model_doc/deformable_detr)) using PyTorch.
...
Based on the script [`run_object_detection.py`](https://github.com/Orvex-Research/Flux/examples/pytorch/object-detection/run_object_detection.py).
...
The resulting model can be seen here: https://huggingface.co/qubvel-hf/qubvel-hf/detr-resnet-50-finetuned-10k-cppe5. The corresponding Weights and Biases report [here](https://api.wandb.ai/links/qubvel-hf-co/bnm0r5ex). Note that it's always advised to check the original paper to know the details regarding training hyperparameters. Hyperparameters for current example were not tuned. To improve model quality you could try:
 ...
 - changing training parameters, such as learning rate, batch size, warmup, optimizer and many more (see [TrainingArguments](https://huggingface.co/docs/flux/main_classes/trainer#flux.TrainingArguments))
...
Based on the script [`run_object_detection_no_trainer.py`](https://github.com/Orvex-Research/Flux/examples/pytorch/object-detection/run_object_detection.py).
...
from flux import AutoImageProcessor, AutoModelForObjectDetection
