...
This directory contains two scripts that demonstrate how to fine-tune [MaskFormer](https://huggingface.co/docs/flux/model_doc/maskformer) and [Mask2Former](https://huggingface.co/docs/flux/model_doc/mask2former) for instance segmentation using PyTorch.
For other instance segmentation models, such as [DETR](https://huggingface.co/docs/flux/model_doc/detr) and [Conditional DETR](https://huggingface.co/docs/flux/model_doc/conditional_detr), the scripts need to be adjusted to properly handle input and output data.
...
This example is based on the script [`run_instance_segmentation.py`](https://github.com/Orvex-Research/Flux/examples/pytorch/instance-segmentation/run_instance_segmentation.py).

The script uses the [🤗 Trainer API](https://huggingface.co/docs/flux/main_classes/trainer) to manage training automatically, including distributed environments.

Here, we show how to fine-tune a [Mask2Former](https://huggingface.co/docs/flux/model_doc/mask2former) model on a subsample of the [ADE20K](https://huggingface.co/datasets/zhoubolei/scene_parse_150) dataset. We created a [small dataset](https://huggingface.co/datasets/qubvel-hf/ade20k-mini) with approximately 2,000 images containing only "person" and "car" annotations; all other pixels are marked as "background."
...
- Adjusting training parameters such as learning rate, batch size, warmup, optimizer, and more (see [TrainingArguments](https://huggingface.co/docs/flux/main_classes/trainer#flux.TrainingArguments))
...
This example is based on the script [`run_instance_segmentation_no_trainer.py`](https://github.com/Orvex-Research/Flux/examples/pytorch/instance-segmentation/run_instance_segmentation_no_trainer.py).
...
from flux import Mask2FormerForUniversalSegmentation, Mask2FormerImageProcessor
