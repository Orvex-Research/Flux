...
> [!WARNING]
> The Flux repo is maintained by Orvex Research. We ask that new contributors verify their contributions locally before submitting PRs.
...
- Compare against other models. The Flux repo is very large, and many models are doing similar things. When
...
Flux welcomes all contributions whether it is fixing bugs, submitting feature requests, implementing new models, or improving docs.

However you choose to contribute, please be mindful and respect our [code of conduct](https://github.com/Orvex-Research/Flux/blob/main/CODE_OF_CONDUCT.md).

If you enjoyed using Flux, feel free to reference it and let us know how you're using it, or give the repository a star!

This guide was heavily inspired by the Hugging Face Flux guide to contributing.
...
1. Fork [Flux](https://github.com/Orvex-Research/Flux) with the **Fork** button on GitHub to get a copy under your account, then clone it locally.

  ```bash
  git clone git@github.com:<your Github handle>/Flux.git
  cd Flux
  ```

2. Add the Flux repository as a remote named `upstream`. `origin` points at your fork and `upstream` points at the source of truth. That lets you sync your local `main` before pushing to `origin` and opening a PR.

  ```bash
  git remote add upstream https://github.com/Orvex-Research/Flux.git
  git remote -v
  # origin    git@github.com:<your Github handle>/Flux.git (fetch)
  # origin    git@github.com:<your Github handle>/Flux.git (push)
  # upstream  https://github.com/Orvex-Research/Flux.git (fetch)
  # upstream  https://github.com/Orvex-Research/Flux.git (push)
  ```
...
Use the issue templates to help you get started.

### Bug-related issue

Make sure the bug wasn't already reported before opening an issue. The issue should be a bug in Flux, not in your own code.

If you're unsure whether the bug is in your code or the library, contact us at research@orvex.com first.
...
Adding a model to Flux makes it available for anyone to load and fine-tune it or run inference. Before you start the implementation, isolate the differences between your architecture and recent state-of-the-art models that are already in Flux. Explain those differences in the issue or PR so reviewers can quickly understand what challenges the model brings, what parts can reuse existing patterns, and what should be standardized across models.

Follow the guides below.

1. [Add a model with modular Flux](docs/source/en/modular_transformers.md) — implement the model using the modular file and generate standalone modeling files.
...
- [Dynamic weight loading](docs/source/en/weightconverter.md) if published checkpoint parameter names do not match the Flux implementation.
...
- Day-0 integration: make the model available in Flux on release day with a new version release. We provide help to support the most important core features (quantization, FlashAttention, KV-cache, etc.), review early drafts, and can also take care of the model implementation depending on timelines.

  Email research@orvex.com a few weeks ahead, especially for novel architectures. We'll iterate with you on a private fork until your checkpoint and release are ready.

- Same-week integration: high-demand models usually land within a week, even when the author doesn't reach out.

  Open an issue with the [new model template](https://github.com/huggingface/flux/issues/new?assignees=&labels=New+model&projects=&template=new-model-addition.yml) to request one. Issues with more activity move up the queue faster.
...
- Post-release integration: models without strong demand, or that we don't have bandwidth to take on, land after the upstream release.
  ...
  Open issues tagged ["New model"](https://github.com/huggingface/flux/issues?q=is%3Aopen+is%3Aissue+label%3A%22New+model%22) are the entry point for outside contributors. Start with the most-requested architectures to maximize impact. We'll review and guide you through it.

- Hub-first release: ship your model directly on the Hub via Flux' [remote-code](./docs/source/en/models#custom-models.md) support, with no upstream PR required.

  Popular Hub-first models often get integrated into Flux later, which unlocks first-class docs, maintenance, and optimization. Hub-first is the lowest-friction way to add a model. However, it can be more fragile if a model requires weight conversion or other backward compatibility issues.
